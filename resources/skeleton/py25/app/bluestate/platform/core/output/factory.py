...
class FCMCoreOutputLoader(JFileSystemLoader):
	...
	@cached_property
	def devConfig(self):
		return config.config.get('bluestate.project.dev')
		
	@cached_property
	def loaderConfig(self):
		return config.config.get('bluestate.project.output.template_loader')
...
def fcmOutputEnvironmentFactory(environment):
	...
	util = {
		...
		'config':{
			...
			'fcm': config.config.get('bluestate.project'),
			'platform': config.config.get('bluestate.platform'),
			'all': config.config,
			'hooks':{
				'appstats': (config.config.get('bluestate.system')['hooks']['appstats']['enabled'] == True or config.config.get('bluestate.services')['hooks']['appstats']['enabled'] == True),
				'apptrace': (config.config.get('bluestate.system')['hooks']['apptrace']['enabled'] == True or config.config.get('bluestate.services')['hooks']['apptrace']['enabled'] == True)
			}
		},
		'env': os.environ,
		'len': len,
