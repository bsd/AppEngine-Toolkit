...
class _BSDMessageJSONEncoder(protojson._MessageJSONEncoder):
	...
	def default(self, value):
		...
		if isinstance(value, messages.Message):
			...
			for field in value.all_fields():
				...
				return super(_BSDMessageJSONEncoder, self).default(value)
		else:
			return super(_BSDMessageJSONEncoder, self).default(value)
...
class BSD_JSONRPC_Mapper(service_handlers.JSONRPCMapper):
	...
	def __init__(self):
		super(BSD_JSONRPC_Mapper, self).__init__()
		
	def encode_request(self, struct):
		encoded = _BSDMessageJSONEncoder().encode(struct)
		return encoded
	...
	def envelope(self, wrap, response):
		...
		sysconfig = config.config.get('bluestate.project')
		if config.debug:
			debugflag = True
		...
		return {
			...
			'platform': {
				...
				'engine': 'BSD/AppTools::v1.1 Embedded'
			}
