...
from bluestate.services.core import handler
from bluestate.services.core import dialects
from bluestate.services.core.dialects import *

from webapp2_extras import protorpc as proto
from protorpc.webapp import service_handlers

from bluestate.services.core.factory.service import BlueStateServiceFactory

## Service layer middleware cache
_middleware_cache = {}


class BlueStateServiceHandlerFactory(proto.ServiceHandlerFactory):
	
	@werkzeug.cached_property
	def servicesConfig(self):
		return config.config.get('bluestate.services')
	...
	def default(cls, service_factory, parameter_prefix=''):
		...
		factory.add_request_mapper(dialects.jsonrpc.BSD_JSONRPC_Mapper())
	...
	def __call__(self, request, remote_path, remote_method):
		...
		service._initializeBlueStateService()
		...
		service_handler = BlueStateServiceFactory.new(handler.BlueStateServiceHandler(self, service))
