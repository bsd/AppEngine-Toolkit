...
from bluestate.services.core.factory.handler import BlueStateServiceHandlerFactory

## Get services configuration
services_config = config.config.get('bluestate.project.services')
...
def generateServiceMappings(svc_cfg, registry_path=forms.DEFAULT_REGISTRY_PATH):
	...
	if svc_cfg is not None and svc_cfg['services'] is not None and len(svc_cfg['services']) > 0:
		...
		for path, service in services:
			...
			new_mapping = BlueStateServiceHandlerFactory.default(service).mapping(path)
...
def disabled():
	print "X-BSDAppTools-Services-Error: SERVICES_DISABLED"		
...
def main(environ=None, start_response=None):
	...
	if services_config is not None and services_config.get('enabled', False) == True:
